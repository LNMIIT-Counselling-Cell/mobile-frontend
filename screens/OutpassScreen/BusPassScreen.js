...
export default function BusPassScreen({ navigation }) {

  const { colors } = useTheme()

  const [numPass, setNumPass] = useState(0);
  const [busPassAvailable, setBusPassAvailable] = useState(20);
  const busPassPrice = 25;
  const [busPassValue, setBusPassValue] = useState(0);
  ...
  onChangeNumPass = (value)=>{
    ...
    setNumPass(newText);
    if (newText !== ''){
      setBusPassValue(parseInt(newText)*busPassPrice);
    }else{
      setBusPassValue(0);
    }
}
    ...
    <ScrollView>
      <View style={styles.container}>
        ...
        <View style={[styles.form, { backgroundColor: colors.cardBG }]}>
          ...
          <Text style={[{ color: colors.text }]}>Bus Passes Available : {busPassAvailable}</Text>
          ...
          <View style={styles.item}>
            <Text style={[{ color: colors.text }]}>Number of passes :</Text>
            <TextInput
              style={[styles.formInput, { color: colors.text }, { backgroundColor: colors.background }]}
              keyboardType='numeric'
              value={numPass}
              onChangeText={(value)=>(onChangeNumPass(value))}
            />
          </View>
          <Text style={[{ color: colors.text }]}>Per Bus Pass Price: ₹{busPassPrice}</Text>
          <Text style={[{ color: colors.text }]}>Bus Pass Net Value: ₹{busPassValue}</Text>
          {/* <View style={styles.item}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            // }
              ...
              }else if (parseInt(numPass) > 100){
                alert("Only 100 bus passes can be made at a time")
              }else if (parseInt(numPass) <= 0){
                alert("Please enter valid number")
              }else{
                alert("That's my boy!!")
              }
        ...
        >
          <View style={styles.btn}>
            ...
            {/* {isLoadingGen ? "Generating ..." : "Generate Outpass"} */}
              Get Bus Pass(es)
            </Text>
          </View>
        </TouchableOpacity>
          ...
          onPress={
            ...
            ()=>{
              if (numPass === ""){
                ...
                alert("Only 100 bus passes can be used at a time")
              }else if (parseInt(numPass) <= 0){
                alert("Please enter valid number")
              }else if(parseInt(numPass) > busPassAvailable){
                alert("Insufficient bus passes available")
              }else{
                alert("That's my boy!!")
              }
            }
        ...
        >
          <View style={styles.btn}>
            ...
            {/* {isLoadingPrev ? "Fetching ..." : "Previous Outpass"} */}
              Use Bus Pass(es)
            </Text>
          </View>
        </TouchableOpacity>
